...
/** Prefer explicit URL; otherwise build from a bare arXiv ID if present. */
...
/** Remove standalone URLs not already used inside markdown link targets. */
...
/** Turn `Author (Year). Title` + hidden URL into `Author (Year). [*Title*](url).` */
...
/** Strip arXiv from the paper body and embed source URLs in reference titles only. */
