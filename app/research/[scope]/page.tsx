...
export default async function LecturerResearchWorkspacePage({ params, searchParams }: Props) {
	const { scope: slug } = await params;
	...
	if (canonical && slug !== canonical) {
		redirect(`/research/${canonical}${query}`);
	}

	return (
		<Suspense fallback={null}>
			<ResearchScopeBriefPage scope={scope} variant="lecturer" />
		</Suspense>
	);
}
