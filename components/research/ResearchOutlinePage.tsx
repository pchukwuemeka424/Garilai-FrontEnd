...
function ResearchOutlineContent({ variant = "lecturer" }: Props) {
	...
	}, [context, hasTokens, persistOutline, setTokenQuota]);
	...
	return (
		<div className={`research-outline-page${isStudent ? " research-outline-page-student" : ""}`}>
					...
					title={
						!outline?.trim()
							...
							: !hasTokens
								...
								: `Draft a full ${getGenerateResearchLabel(context?.scope).replace(/^Generate\s+/i, "")} from this outline`
					}
				>
					<IconFileText size={14} />
					{getGenerateResearchLabel(context?.scope)}
				</button>
				{outline && !loading ? (
			...
			<ResearchCitationStyleModal
				...
				projectTitle={context.idea.title}
				variant={isStudent ? "student" : "lecturer"}
				note="Your outline will guide section structure. Citations and the References list will use your chosen style."
				confirmLabel={getGenerateResearchLabel(context?.scope)}
			/>
		</div>
	);
}
